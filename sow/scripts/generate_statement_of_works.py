...
def calculate_contract_fee(day_rate, number_of_months, prorata_for_last_payment):
    ...
    if prorata_for_last_payment > 0:
        ...
        return round(day_rate * paid_working_days_in_month * number_of_months, 2)


def calculate_retention_fee(contract_fee):
    retention_fee_percentage = 0.05

    return round(contract_fee * retention_fee_percentage, 2)


def calculate_base_monthly_payment(contract_fee, number_of_payments):
    return round((contract_fee / number_of_payments) * 0.95, 2)
...
def generate_statement_of_works(
    ...
    contract_length = calculate_contract_length(parsed_start_date, parsed_end_date)
